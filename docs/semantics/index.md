...
8. **[Weighted deduction fragment](grammar.md).** Item algebras, rule systems as hyperedges in a multicategory, semiring-weighted chart enumeration, axiom injectors, strategy independence, and differentiable charts. The deduction abstraction subsumes categorical grammars, type-theoretic proof systems, Datalog-shaped fixed-point evaluations, and graph algorithms as parameter settings on a single agenda engine.
