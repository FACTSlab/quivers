...
By induction, $\llbracket e_1 \rrbracket \in \mathrm{Hom}_{\mathbf{Kern}}(\Phi \times A,\, \mathcal{G}(B))$ and $\llbracket e_2 \rrbracket \in \mathrm{Hom}_{\mathbf{Kern}}(\Phi \times B,\, \mathcal{G}(C))$. To compose, we thread $\Phi$ unchanged through $e_1$ using the strength $\mathrm{str}_{\Phi, B} : \Phi \times \mathcal{G}(B) \to \mathcal{G}(\Phi \times B)$ of $\mathcal{G}$ on $\mathbf{SBor}$ ([Programs §5](programs.md#5-soundness-of-monadic-semantics)):

$$
\widetilde{e_1} \;=\; \mathrm{str}_{\Phi, B} \circ \langle \pi_\Phi,\ \llbracket e_1 \rrbracket \rangle
\;:\; \Phi \times A \;\to\; \mathcal{G}(\Phi \times B).
$$

Then $\llbracket e_1 \mathbin{\diamond_\alpha} e_2 \rrbracket = \llbracket e_2 \rrbracket \diamond \widetilde{e_1} : \Phi \times A \to \mathcal{G}(C)$, where the outer $\diamond$ is Kleisli composition in $\mathbf{Kern}$. The result lies in $\mathrm{Hom}_{\mathbf{Kern}}(\Phi \times A,\, \mathcal{G}(C))$, the required signature. Well-definedness of $\diamond$ is the categorical structure of $\mathbf{Kern}$ ([Setting §3](setting.md#3-standard-borel-spaces-and-markov-kernels)); strength coherence is the standard Kock-Linton presentation of $\mathcal{G}$ as a strong monad on $\mathbf{SBor}$ ([Fritz 2020 §4](https://doi.org/10.1016/j.aim.2020.107239)).

*Case $\textsc{Bind}$.* By induction, $\llbracket F(\bar a) \rrbracket \in \mathrm{Hom}_{\mathbf{Kern}}(\Phi,\, \mathcal{G}(B))$. The denotation of the bind statement is the kernel given on measurable rectangles $E \times C$ with $E \in \Sigma_\Phi$ and $C \in \Sigma_B$ by

$$
\mathcal{S}\llbracket v \leftarrow F(\bar a) \rrbracket(\phi,\, E \times C)
\;=\;
\mathbf{1}_{E}(\phi) \cdot \llbracket F(\bar a) \rrbracket(\phi)(C),
$$

extended to all of $\Sigma_{\Phi \times B}$ by Caratheodory's theorem (the rectangles generate the product $\sigma$-algebra, and the function $\phi \mapsto \mathbf{1}_E(\phi) \cdot \llbracket F(\bar a) \rrbracket(\phi)(C)$ is measurable because $\llbracket F(\bar a) \rrbracket$ is a Markov kernel). The resulting measurable map $\Phi \to \mathcal{G}(\Phi \times B)$ is the conclusion of $\textsc{Bind}$ at the trace context $\Phi, v : B$. The categorical content is that the right-product-fibration $- \times B : \mathbf{Kern} \to \mathbf{Kern}$ lifts every Kleisli arrow $\Phi \to \mathcal{G}(B)$ to a kernel $\Phi \to \mathcal{G}(\Phi \times B)$ via the strength used in the $\textsc{Compose}$ case.

*Case $\textsc{Marginalize}$.* By induction, the body denotes a Kleisli arrow $k_{\mathrm{body}} : \llbracket \Phi, v : B \rrbracket \to \mathcal{G}(\llbracket \Phi' \rrbracket)$. Thread $\Phi$ through $\llbracket F(\bar a) \rrbracket : \Phi \to \mathcal{G}(B)$ via the same strength construction as $\textsc{Compose}$ to obtain $\widetilde{F} : \Phi \to \mathcal{G}(\Phi \times B)$. Kleisli-compose with $k_{\mathrm{body}}$ to obtain $\Phi \to \mathcal{G}(\Phi')$. Finally, the pushforward $\pi_*$ along the measurable projection $\pi_{\Phi' \setminus v} : \Phi' \to \Phi' \setminus v$ (defined on measures by $\pi_* \mu(F) = \mu(\pi^{-1} F)$ for $F \in \Sigma_{\Phi' \setminus v}$) post-composes to yield a kernel $\Phi \to \mathcal{G}(\Phi' \setminus v)$, the statement's required signature. The pushforward is the integral over the $v$-fibre (Fubini-Tonelli on the product $\sigma$-algebra), which is the measure-theoretic content of marginalisation.
...
*Proof.* Induction on $e$. Each Expr AST variant has a unique introduction rule, modulo the discriminator on $\textsf{ExprCompose}$'s `op` field which selects between $\textsc{ComposePoly}_\alpha$ and $\textsc{ComposeTag}_\beta$ (the `op` value determines the rule, hence determines $\alpha$ or $\beta$). The unique rule determines the conclusion's $A$ and $B$ as functions of the premises' types: e.g. $\textsc{ComposePoly}_\alpha$ fixes the conclusion's $A$ to be the first premise's $A$ and the conclusion's $B$ to be the second premise's $C$. The premises themselves have unique types by induction. Base cases: $\textsc{ModuleVar}$ reads $A \rightsquigarrow B$ uniquely from $\Gamma$ by the disjointness of context entries (a single name binds to a single declared signature); $\textsc{TraceVar}$ reads $\tau$ uniquely from $\Phi$ by the same disjointness on $\Phi$. $\square$
...
* Morphism-expression rules: the AST variant of $e$ (one of $\textsf{ExprIdent}$, $\textsf{ExprIdentity}$, $\textsf{ExprCompose}$, $\textsf{ExprTensorProduct}$, $\textsf{ExprFan}$, $\ldots$) partitions them. $\textsf{ExprCompose}$ is further split on its `op` field: the polymorphic `op` $\in \{\mathtt{>>}, \mathtt{<<}, \mathtt{>=>}\}$ selects $\textsc{ComposePoly}_\alpha$ (with $\alpha$ the module's declared algebra); the tagged `op` $\in \{\mathtt{*>}, \mathtt{\sim>}, \mathtt{||>}, \mathtt{?>}, \mathtt{\&\&>}, \mathtt{+>}, \mathtt{\$>}, \mathtt{\%>}\}$ selects the corresponding $\textsc{ComposeTag}_\beta$.
