...
The full proof-calculus presentation, with every judgment form, the inference-rule set, the soundness theorem, subject reduction for parametric instantiation, and the bidirectional algorithm underlying the compiler's typechecker, is the subject of the [Typing](typing.md) chapter. The implementation in `quivers.dsl.compiler` is its operational realisation, and the [Adequacy](adequacy.md) theorem establishes the soundness of typing with respect to the denotation.
