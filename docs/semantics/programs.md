...
* **Strength coherence.** $\mathcal{G}$ is a *commutative strong* monad on $\mathbf{SBor}$: the canonical strength $\mathrm{str}_{S, T} : S \times \mathcal{G}(T) \to \mathcal{G}(S \times T)$ given on rectangles by $(s, m) \mapsto \delta_s \times m$ is well-defined and satisfies the Kock-Linton strength axioms ([Fritz 2020, §4](https://doi.org/10.1016/j.aim.2020.107239)). The stated coherence is the naturality of $\mathrm{str}$ in its arguments, which holds for any strong monad. Commutativity (the equation $\mathrm{str}' \diamond \mathrm{str} = \mathrm{str} \diamond \mathrm{str}'$ relating the left and right strengths) is the substance of the "independent draws commute" claim; for $\mathcal{G}$ on $\mathbf{SBor}$ it is the Fubini-Tonelli theorem on the product $\sigma$-algebra.
