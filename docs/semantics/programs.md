...
* **Strength coherence.** $\mathcal{G}$ is a *commutative strong* monad on $\mathbf{SBor}$: the canonical right strength $\mathrm{str}_{S, T} : S \times \mathcal{G}(T) \to \mathcal{G}(S \times T)$ given on rectangles by $(s, m) \mapsto \delta_s \times m$, and the corresponding left strength $\mathrm{str}'_{S, T} : \mathcal{G}(S) \times T \to \mathcal{G}(S \times T)$, are both well-defined and jointly satisfy the four Kock-Linton strength axioms ([Fritz 2020, §4](https://doi.org/10.1016/j.aim.2020.107239)). The strength-coherence row of the table is the standard naturality of $\mathrm{str}$ with respect to the symmetric-monoidal swap of $\mathbf{SBor}$; the proof is the usual diagram chase for any strong monad. Crucially, *commutativity* of the two strengths,
$$
\mu \circ \mathcal{G}(\mathrm{str}') \circ \mathrm{str} \;=\; \mu \circ \mathcal{G}(\mathrm{str}) \circ \mathrm{str}' \quad \text{as maps}\ \mathcal{G}(S) \times \mathcal{G}(T) \to \mathcal{G}(S \times T),
$$
is the categorical content of the "independent draws commute" claim that lets a QVR program reorder unrelated bind statements without changing its denotation. For $\mathcal{G}$ on $\mathbf{SBor}$, the commutativity equation is exactly Fubini-Tonelli on the product $\sigma$-algebra ([Kock 1972](https://doi.org/10.7146/math.scand.a-11434)).
