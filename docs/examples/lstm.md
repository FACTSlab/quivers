...
The four `<-` bind steps sample from the gate and candidate morphisms. Each bind applies the morphism and names the result for subsequent use, both sampling parameters from the prior and applying the morphism.
...
- **Bind steps**: `name <- morphism(args)` applies a stochastic morphism, samples from its distribution, and binds the result. Multiple binds compose through the probability monad.
