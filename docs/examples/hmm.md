...
The forward algorithm and the Viterbi algorithm are the same composed morphism evaluated in different [quantales](https://ncatlab.org/nlab/show/quantale): under product, composition multiplies probabilities and summation marginalizes; under tropical, composition adds log-probabilities and summation maximizes. Quivers makes this explicit: switching `quantale` changes the V-enriched composition rule without touching the program text.
