...
The [deep Markov model](https://doi.org/10.1609/aaai.v31i1.10779) of Krishnan, Shalit, and Sontag (2017) is a state-space model with nonlinear, neural-network-parameterised transition and emission kernels:
...
The transition and emission means are MLPs; per-step Normal noise gives a tractable density. The companion recognition network `q_\phi(o_t, s_{t-1}) -> s_t` carries the variational posterior and is threaded across the sequence by `scan` to amortise the posterior over the latent trajectory. The combinator surface mirrors the [linear-Gaussian SSM](linear-gaussian-ssm.md): only the per-step cells change.
...
print("recognised state shape:", final_state.shape)

# Smoke-train the recogniser to align with the generative prior; in
# a full setup, ``condition`` the generative pipeline on ``o_seq``
# and fit by SVI with the recogniser as the amortised guide.
