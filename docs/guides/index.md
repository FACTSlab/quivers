...
3. **[Morphisms & Composition](morphisms.md).** What a morphism is as a tensor in $\mathcal{V}^{|A| \times |B|}$. The morphism hierarchy, composition, tensor product, marginalisation, and the compact-closed surface (`dagger`, `trace`, `cup`, `cap`).
...
8. **[Continuous Distributions](continuous.md).** ContinuousSpace and ContinuousMorphism: parameterised families, sampled composition, normalising flows.
