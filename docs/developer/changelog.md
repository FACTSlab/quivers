...
- **`scan(...)` is refused rather than approximated.** It had no lowering: the expansion pass encoded it as an elementwise sum that surfaced the cell as a free data input, so the emitted program computed the hidden state by adding two data arrays, carried no sample site for the recurrence, and left a log density independent of the chain's parameters. A scan denotes one draw per sequence position over intermediate states the program never names, over a sequence axis that arrives with the data, so there is no loop bound to emit and no name to bind the states to. The four sequence models that rendered this way now report that, naming the cell and the two ways to write a recurrence that does transpile.
- **A target refuses a construct it would score as a different measure.** Gen has no way to add a free log-density term to a trace, so it emitted every `marginalize` as a live draw, denoting a measure on the product of the latent's support with the block's; it now refuses. BUGS, Church, Edward2, JAGS, PyMC and Turing reduce an ungrouped marginalize per row and refuse that case for the same reason, while Stan, NumPyro, Pyro and WebPPL emit the accumulated order. Until a target can express the measure, it refuses rather than emitting a score for a different one.
