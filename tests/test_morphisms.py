...
from quivers.core.morphisms import ComposedMorphism, ProductMorphism, MarginalizedMorphism, morphism, observed
...
class TestComposition:
    ...
    def test_boolean_composition(self):
        ...
        h = f >> g
        expected = torch.tensor([[1.0, 0.0, 1.0], [0.0, 1.0, 0.0], [1.0, 1.0, 1.0]])
        torch.testing.assert_close(h.tensor, expected, atol=1e-05, rtol=1e-05)
            ...
            for k in range(2):
                for j in range(2):
                    for l in range(2):
                        ...
                        torch.testing.assert_close(t[i, k, j, l], expected, atol=1e-06, rtol=1e-06)
...
class TestFreeMonoidComposition:
    ...
    def test_compose_through_free_monoid(self):
        ...
        fm_b = FreeMonoid(generators=b, max_length=2)
        f = morphism(a, fm_b)
        g = morphism(fm_b, a)
        h = f >> g
        assert h.domain == a
    ...
    def test_product_with_free_monoid(self):
        ...
        a = FinSet(name='A', cardinality=2)
        b = FinSet(name='B', cardinality=3)
        fm_a = FreeMonoid(generators=a, max_length=1)
        f = morphism(fm_a, b)
        g = morphism(fm_a, b)
        fg = f @ g
...
class TestMarginalizationAsDecomposition:
    ...
    def test_marginalize_preserves_monotonicity(self):
        ...
        bcd = ProductSet(components=(b, c, d))
        data = torch.rand(3, 4, 2, 5)
        f = observed(x, bcd, data)
        g = f.marginalize(b)
        marginalized = g.tensor
        for b_idx in range(4):
            assert (data[:, b_idx, :, :] <= marginalized + 1e-06).all()
