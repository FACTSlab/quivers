...
class TestFreeMonoidMonad:
    ...
    def test_multiply_flattens(self):
        ...
        a = FinSet(name='A', cardinality=2)
        mu = m.multiply(a)
        fm_a = FreeMonoid(generators=a, max_length=2)
        inner_idx = fm_a.encode((0,))
        fm_fm_a = FreeMonoid(generators=FinSet(name=f'{a.name}*', cardinality=fm_a.size), max_length=2)
    ...
    def test_unit_requires_finset(self):
        ...
        with pytest.raises(TypeError, match='FinSet'):
            m.unit(FreeMonoid(generators=FinSet(name='A', cardinality=2), max_length=1))
...
class TestMonadLaws:
    ...
    def test_fuzzy_powerset_left_unit(self):
        ...
        torch.testing.assert_close(result, torch.eye(3), atol=1e-05, rtol=1e-05)
