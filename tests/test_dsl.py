...
from quivers.dsl.ast_nodes import Module, QuantaleDecl, CategoryDecl, RuleDecl, CatPatternName, CatPatternSlash, CatPatternProduct, ObjectDecl, MorphismDecl, SpaceDecl, ContinuousMorphismDecl, StochasticMorphismDecl, DiscretizeDecl, EmbedDecl, DrawStep, LetStep, LetExprLiteral, LetExprVar, ProgramDecl, LetDecl, OutputDecl, TypeName, TypeProduct, TypeCoproduct, SpaceConstructor, SpaceName, SpaceProduct, ExprIdent, ExprIdentity, ExprCompose, ExprTensorProduct, ExprMarginalize, ExprFan, ExprRepeat, ExprStack, ExprScan, ExprParser
...
class TestParserContinuous:
    ...
    def test_discretize_decl(self):
        ...
        mod = self._parse('space B : Euclidean(1, low=0.0, high=1.0)\ndiscretize d : B -> 10')
        stmt = mod.statements[1]
...
class TestContinuousDSLIntegration:
    ...
    def test_all_families_via_dsl(self):
        ...
        unbounded_families = ['Normal', 'LogitNormal', 'Beta', 'Cauchy', 'Laplace', 'Gumbel', 'LogNormal', 'StudentT', 'Exponential', 'Gamma', 'Chi2', 'HalfCauchy', 'HalfNormal', 'InverseGamma', 'Weibull', 'Pareto', 'Kumaraswamy', 'ContinuousBernoulli', 'FisherSnedecor']
...
class TestCompilerProgram:
    ...
    def test_gradient_flow(self):
        ...
        loss = out.sum()
        loss.backward()
        has_grad = any((p.grad is not None and p.grad.abs().sum() > 0 for p in prog.parameters()))
        assert has_grad
...
class TestConditionalBernoulli:
    ...
    @pytest.mark.xfail(reason='blocked on panproto/panproto#87 (mixed-domain ProductSpace)', strict=True)
...
class TestExecutionLetSteps:
    ...
    def test_faithful_pds_factivity(self):
        ...
        result = prog.rsample(entity)
        assert isinstance(result, dict)
        assert set(result.keys()) == {'tau_know', 'cg_complement', 'cg_matrix', 'response'}
    ...
    def test_faithful_pds_log_joint(self):
        ...
        entity = torch.tensor([0, 1])
        result = prog.rsample(entity)
        lj = prog.log_joint(entity, {'belief': result['belief'], 'truth': result['truth'], 'cg_status': result['cg_status']})
        assert lj.shape == (2,)
        assert torch.isfinite(lj).all()
        lj2 = prog.log_joint(entity, {'theta': result['belief'], 'b': result['truth'], 'cg': result['cg_status']})
