...
def test_encoder_default_scaffold_compiles():
    ...
    encoder MyComp over S { dim Term = 16 }
    """
    prog = loads(src)
...
def test_decoder_default_scaffold_samples_and_scores():
    ...
    encoder C over S { dim Term = 16 }
    decoder D over S depth 4 { body |-> recursive }
    """
    prog = loads(src)
...
def test_loss_decl_registers_and_evaluates():
    src = """
    signature S {
        sorts { Term : object dim 8 }
        constructors { A : -> Term, B : Term -> Term }
    }
    encoder C over S { dim Term = 8 }
    decoder D over S depth 3 { body |-> recursive }
    loss zero weight 2.0 on encoder C { 1.0 + 2.0 }
    """
    prog = loads(src)
...
def test_rule_attached_loss_fires_on_each_rule_application():
    src = """
    signature Items {
        sorts { Item : object dim 4 }
        constructors { NP : -> Item, S : -> Item }
    }
    encoder C over Items { dim Item = 4 }

    deduction D : Item -> Item {
        atoms { NP, S }
        rule app : NP, NP |- S
        semiring LogProb
        signature Items
    ...
    loss per_app weight 1.0 on rule app in D { 0.5 }
    """
    prog = loads(src)
    D = prog.deductions["D"]
    import torch as _t
    ...
    # the popped item is premise 0 and once when it's premise 1, each
    # with the same single sibling); each firing accumulates `0.5`.
...
def test_chart_attached_loss_fires_after_deduction_completes():
    ...
    loss completed weight 1.0 on chart of D { 7.0 }
    """
    prog = loads(src)
    D = prog.deductions["D"]
    import torch as _t
...
def test_multiple_losses_attached_at_different_sites():
    ...
    encoder C over S { dim Term = 8 }
    decoder D over S depth 3 { body |-> recursive }
    loss a weight 1.0 on encoder C { 1.0 }
    loss b weight 3.0 on decoder D { 2.0 }
    loss c { 5.0 }
    """
    prog = loads(src)
    assert prog.losses is not None
...
def test_data_sort_vocab_declared_in_signature_block():
    ...
    signature Phrase {
        sorts { Phrase : object dim 16 }
    }
    """
    # Two separate signatures wouldn't share sorts; keep things
    # simple with a single signature that names Tok and Phrase.
    src = """
    signature LM {
        sorts {
            Tok    : data dim 16 vocab { "a", "b", "c", "d" }
    ...
    encoder C over LM { dim Tok = 16, dim Phrase = 16 }
    decoder D over LM depth 3 { body |-> recursive }
    """
    prog = loads(src)
...
def test_vocab_clause_only_valid_on_data_sorts():
    src = """
    signature S {
        sorts { Term : object dim 8 vocab { "x" } }
    }
    """
    import pytest
    from quivers.dsl import CompileError
...
def test_duplicate_vocab_entry_rejected():
    src = """
    signature S {
        sorts { Tok : data dim 8 vocab { "a", "b", "a" } }
    }
    """
    import pytest
    from quivers.dsl import CompileError
...
def test_binder_threads_de_bruijn_context():
    ...
    encoder C over STLC { dim Term = 16, dim Type = 8 }
    """
    prog = loads(src)
    C = prog.encoders["C"]
...
def test_deduction_with_attached_encoder_exposes_embeddings():
    ...
    encoder C over Items { dim Item = 8 }

    deduction Parse : Item -> Item {
        atoms { S, NP }
