...
def _walk_return_pattern(
    ...
    if k == "return_labeled_tuple":
        entries = [
            e for e in t.positional(vid) if t.kind(e) == "return_label_entry"
        ]
        names: list[str] = []
        labels: list[str] = []
        for entry in entries:
            ...
            if label_vid is None or var_vid is None:
                raise ParseError(
                    f"return_label_entry missing field at {entry}"
                )
...
def _walk_program_param(t: _Tree, vid: str) -> ProgramParam:
    ...
    if k == "identifier":
        ...
        return ObjectParam(
            name=t.text(vid), universe="Object", line=line, col=col,
        )
    if k == "typed_program_param":
        ...
        if name_vid is None or kind_vid is None:
            raise ParseError(
                f"typed_program_param missing name/kind at {vid}"
            )
        name = t.text(name_vid)
        kk = t.kind(kind_vid)
        if kk == "object_kind":
            ...
            return ObjectParam(
                name=name,
                universe=cast(
                    'Literal["FinSet", "Space", "Object"]', universe_text
                ),
                line=line,
                col=col,
            )
        ...
        if kk == "morphism_kind":
            ...
            if dom_vid is None or cod_vid is None:
                raise ParseError(
                    f"morphism_kind missing domain/codomain at {kind_vid}"
                )
