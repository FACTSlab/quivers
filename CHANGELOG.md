...
## [Unreleased] - transpile development

The transpile pipeline (structural IR, per-family metadata, target
renderers for Stan / NumPyro / Pyro / PyMC / Edward2 / Turing / Gen
/ WebPPL / BUGS / JAGS / Church) is staged on this branch and does
not ship in 0.15.0. It will land in a subsequent release; the
version marker on this branch is ``0.16.0.dev0`` to reflect that
target.

Progress accumulates directly in ``src/quivers/transpile/`` and the
supporting ``grammars/qvr/`` extensions; see the individual commit
history on ``feat/transpile-*`` branches for the incremental
work. This entry is a placeholder until the pipeline is ready to
ship, at which point the entries below will migrate under the
