...
- **A target refuses a construct it would score as a different measure.** Gen has no way to add a free log-density term to a trace, so it emitted every `marginalize` as a live draw, denoting a measure on the product of the latent's support with the block's; it now refuses. BUGS, Church, Edward2, JAGS, PyMC and Turing reduce an ungrouped marginalize per row and refuse that case for the same reason, while Stan, NumPyro, Pyro and WebPPL emit the accumulated order. Refusing is the honest outcome while a target cannot express the measure: a wrong number no comparison catches is worse than none.
- **The vanilla RNN is written as the recurrence it is.** `vanilla_rnn_lm` hid its transition in a morphism carrying `[param_source=mlp]`, whose weights are model-internal, so it was the only one of the three sequence models no backend would take. Its cell is now a program whose step is a declared site, and its update is the single-matrix `tanh` a vanilla RNN is defined by rather than a two-hidden-layer network standing in for one.
